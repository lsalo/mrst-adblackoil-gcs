classdef HystereticRelativePermeability < StateFunction
    properties
...
%        regions_imbibition = [];
    ...
    methods
        function gp = HystereticRelativePermeability(varargin)
            gp@StateFunction(varargin{:});
            gp = gp.dependsOn({'s', 'sMax'}, 'state');
%             % LS >
%             if nargin > 2
%                gp.regions_imbibition = varargin{3}; % third input must be imb 
%             end % LS <
        end
        function kr = evaluateOnDomain(prop, model, state)
            ...
            if nph < 2
                ...
                elseif model.gas && model.water %LS >
                    kr = prop.relPermWG(model, state); % < LS
                else
    ...
    function kr = relPermWG(prop, model, state)  % LS >
        % Water-gas system
        [sw, sg] = model.getProps(state, 'sw', 'sg');
        sgMax = model.getProps(state, 'sgMax');
        sgMax = max(sg, sgMax); % Set to maximum value of historic maximum and current value
        %f = model.fluid;
        krW = evaluatePhaseRelativePermeability(prop, model, 'w', sw);
    ...
    function kr = relPermOG(prop, model, state)
        ...
        krG = evaluatePhaseRelativePermeabilityWithHysteresis(prop, model, 'g', sg, sgMax); % LS
        if isfield(f, 'krO')
            krO = prop.evaluateFunctionOnDomainWithArguments(f.krO, so);
        else
        ...
        if isfield(f, 'krHyst')
            ...
            if f.krHyst == 1  % hysteresis active for all cells
                ...
                for reg = 1:numel(unique(regions))
                    ...
                    if any(isRegi)
                        kr(idCells) = evaluateFunctionCellSubsetReg(prop, fni{reg}, regions, s(idCells), sMax(idCells));
                    else
